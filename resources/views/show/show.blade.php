...
@section('content')
    <article>
        <h1 class="my-5">{{ $show->title }}</h1>
        <div class="row">
            <div class="col-4">
                @if($show->poster_url)
                    <p><img src="{{ asset('/storage/'. $show->poster_url) }}" alt="{{ $show->title }}" width="250"></p>
                @else
            ...
            <div class="row">
                <div class="col-12">
                    ...
                    <h2 class="my-4 font-weight-500">Buy your ticket now !</h2>
                    @if($show->bookable && $show->representations->count() > 0)
                        <form action="{{ route('cart.store') }}" method="post">
                            ...
                            <div class="mb-2">
                                ...
                                <input type="number" max="30" min="1" value="1" name="order_quantity"
                                       id="order_quantity">
                                <span> seat(s)</span>
                            </div>
